...
$vys_celk = mysqli_query($db,"select id from clanky where stav = 'v' and datum <= $cas");
...
if ($vys_celkem != "0") {
  ...
  echo "<table><tr><td>Vybrat jen èlánky z rubriky:</td><td><form action=\"cl_vydane.php\"><div>\n";
  echo "<select name=\"id\">\n";
...
if (isset($_GET["id"])) {
  ...
  $vysledek_celk = mysqli_query($db,"select id from clanky where id_rubrika = '$id_r' and stav = 'v' and datum <= $cas");
  $vysledek = mysqli_query($db,"select c.id,c.nadpis,c.perex,r.rubrika,a.jmeno from clanky c, rubriky r, autori a where c.id_rubrika = r.id and c.id_autor = a.id and c.stav = 'v' and id_rubrika = '$id_r' and datum <= $cas order by c.datum desc limit $zaznam, $pocet");
  ...
  $vysledek_celk = mysqli_query($db,"select id from clanky where stav = 'v' and datum <= $cas");
  $vysledek = mysqli_query($db,"select c.id,c.nadpis,c.perex,r.rubrika,a.jmeno from clanky c, rubriky r, autori a where c.id_rubrika = r.id and c.id_autor = a.id and c.stav = 'v' and datum <= $cas order by c.datum desc limit $zaznam, $pocet");
